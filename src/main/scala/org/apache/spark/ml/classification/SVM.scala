...
class SVM (
  ...
  override protected def train(dataset: Dataset[_]): SVMModel = {
    ...
    val subModels = lpData.map{ iter => trains(Iterator((iter)))}.cache()
    if ($(kernelType) == "linear") {
     
      val w = Statistics.colStats(subModels.map((_.asInstanceOf[SVMLinearModel].weight))).mean
          
          //_.asInstanceOf[SVMLinearModel].weight)).mean
      val b = subModels.map(_.asInstanceOf[SVMLinearModel].b).mean()
      new SVMLinearModel(uid, w, b) 
    }
    else {
      ...
      new SVMRbfModel(uid, alphaArray, dataArray, b)
    }
  }


  private def trains  (dataset: Iterator[LabeledPoint]): SVMModel = {
    val data = dataset.map(p => LabeledPoint(if(p.label == 1) 1.0 else -1.0, p.features)).toArray
    ...
    while(passes < numPasses && iter < $(maxIter)) {
      ...
      iter+= 1
println("iter number %d, alphaChanged = %d", iter, alphaChanged);
    ...
    if($(kernelType) == "linear"){
      ...
      for(j <- 0 until D) {
       ...
       new SVMLinearModel(uid,org.apache.spark.mllib.linalg.Vectors.dense(w), b)
    }
    else{
...
class SVMLinearModel private[ml] (
    override val uid: String,
    val weight: org.apache.spark.mllib.linalg.Vector,
    val b: Double)
